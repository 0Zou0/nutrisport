...
const ALL_DAYS: DayConfig[] = [
  ...
  {
    date: '2026-04-10',
    trainings: [{ id:'ts-0410a', slot:TrainingSlot.MORNING, type:'Récupération', intensity:TrainingIntensity.LOW, durationMin:30 }],
    orientations: [
  ...
  {
    date: '2026-04-11',
    trainings: [{ id:'ts-0411a', slot:TrainingSlot.AFTERNOON, type:'Match', intensity:TrainingIntensity.HIGH, durationMin:80 }],
    orientations: [
  ...
  {
    date: '2026-04-17',
    trainings: [{ id:'ts-0417a', slot:TrainingSlot.MORNING, type:'Récupération active', intensity:TrainingIntensity.LOW, durationMin:35 }],
    orientations: [
  ...
  {
    date: '2026-04-18',
    trainings: [{ id:'ts-0418a', slot:TrainingSlot.AFTERNOON, type:'Match', intensity:TrainingIntensity.HIGH, durationMin:80 }],
    orientations: [
  ...
  {
    date: '2026-04-21',
    trainings: [{ id:'ts-0421a', slot:TrainingSlot.MORNING, type:'Cardio', intensity:TrainingIntensity.MODERATE, durationMin:50 }],
    orientations: [
  ...
  {
    date: '2026-04-24',
    trainings: [{ id:'ts-0424a', slot:TrainingSlot.MORNING, type:'Récupération', intensity:TrainingIntensity.LOW, durationMin:30 }],
    orientations: [
  ...
  {
    date: '2026-04-25',
    trainings: [{ id:'ts-0425a', slot:TrainingSlot.AFTERNOON, type:'Match', intensity:TrainingIntensity.HIGH, durationMin:80 }],
    orientations: [
...
async function seedDays(days: DayConfig[]) {
  let inserted = 0;
  const errors: string[] = [];
  ...
  for (const day of days) {
    try {
      ...
      for (const opt of allOptions) {
        await prisma.menuOption.upsert({ where:{ id:opt.id }, create:opt, update:{} });
      }
...
// Chaque GET traite un batch (paramètre week=1..4)
export async function GET(req: Request) {
  const url   = new URL(req.url);
  const batch = Number(url.searchParams.get('batch') ?? '1');

  const batches: Record<number, DayConfig[]> = {
    1: ALL_DAYS.slice(0,  7),   // 6-12 avr
    2: ALL_DAYS.slice(7, 14),   // 13-19 avr
    3: ALL_DAYS.slice(14, 21),  // 20-26 avr
    4: ALL_DAYS.slice(21),      // 27-30 avr
  };

  const days = batches[batch];
  if (!days) return NextResponse.json({ error: 'batch doit être 1,2,3 ou 4' }, { status: 400 });

  const result = await seedDays(days);
  return NextResponse.json({ ok: result.errors.length === 0, batch, ...result });
}
