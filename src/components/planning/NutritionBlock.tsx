    ...
    <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-slate-800">Orientation nutritionnelle</h2>
        </div>
        {editable && (
      ...
      <div className="p-4">
        {orientations.length === 0 ? (
          <p className="text-sm text-slate-400 italic">Aucune orientation définie</p>
        ) : (
          <div className="flex flex-col gap-2">
            {orientations.map((o) => (
              <div key={o} className="flex items-center gap-3">
                <Badge className={`${ORIENTATION_COLORS[o]} text-sm`}>
                  {o}
                </Badge>
                <span className="text-sm text-slate-600">{ORIENTATION_LABELS[o]}</span>
              </div>
            ))}
          </div>
        )}
